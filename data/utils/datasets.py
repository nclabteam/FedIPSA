...
class Shakespeare(Dataset):
    def __init__(
        ...
        self.data_transform = lambda x: torch.Tensor(x).type(torch.int64)
        
    def __getitem__(self, index):
        data, targets = self.data[index], self.targets[index]
        if self.data_transform is not None:
            data = self.data_transform(data)
            targets = self.data_transform(targets)

        return data, targets

    def train(self):
        self.data_transform = self.data_transform
        self.target_transform = self.data_transform
...
class HAR(BaseDataset):
    def __init__(
        ...
        if not os.path.isfile(root / "data.npy") or not os.path.isfile(
            ...
            raise RuntimeError(
                "Please run generate_data.py -d har for generating the data.npy and targets.npy first."
            )
