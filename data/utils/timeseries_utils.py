...
def standard_scaling(data, mean, std):
    # mean = np.numpy.array(mean)
    # std = np.numpy.array(std)
    with np.errstate(divide="ignore", invalid="ignore"):
        scaled_data = (data - mean) / std
        scaled_data[~np.isfinite(scaled_data)] = 0.0  # set inf and NaN to 0
    return scaled_data
...
class Augment(object):
    def __init__(
        self,
        jitter_sigma=0, jitter_prob=0,
        shift_window=200, shift_prob=1.0,
        twarp_sigma=0.05, twarp_knots=4, twarp_prob=1.0,
        mwarp_sigma=0.05, mwarp_knots=2, mwarp_prob=1.0,
    ):
