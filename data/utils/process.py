...
from data.utils.datasets import FEMNIST, CelebA, Synthetic, Shakespeare, HAR
# from data.utils.language_utils import process_x, process_y
from data.utils.timeseries_utils import min_max_scaling, Augment
...
def process_shakespeare(args, partition: dict, stats: dict):
    train_dir = DATA_ROOT / "shakespeare" / "data" / "train"
    test_dir = DATA_ROOT / "shakespeare" / "data" / "test"
    client_cnt = 0
    data_cnt = 0
    all_data = []
    all_targets = []
    data_indices = {}
    clients_4_train, clients_4_test = None, None
    with open(DATA_ROOT / "shakespeare" / "preprocess_args.json", "r") as f:
        preprocess_args = json.load(f)
    ...
    if preprocess_args["t"] == "sample":
        ...
        for train_js_file, test_js_file in zip(train_filename_list, test_filename_list):
            ...
            for writer in train["users"]:
                ...
                data = train_data + test_data
                targets = train_targets + test_targets
                all_data.append(np.array(process_x(data)))
                all_targets.append(np.array(process_y(targets)))
                data_indices[client_cnt] = {
                    "train": list(range(data_cnt, data_cnt + len(train_data))),
                    "val": [],
                    "test": list(
                ...
                data_cnt += len(data)
                client_cnt += 1
                print(f"Processed client {client_cnt}.")
    ...
    np.save(DATA_ROOT / "shakespeare" / "data", np.concatenate(all_data))
    np.save(DATA_ROOT / "shakespeare" / "targets", np.concatenate(all_targets))
...
def process_ucihar(args, partition: dict, stats: dict):
    train_dir = DATA_ROOT / "ucihar" / "UCI HAR Dataset" / "train" / "train.csv"
    test_dir = DATA_ROOT / "ucihar" / "UCI HAR Dataset" / "test" / "test.csv"
    client_cnt = 0
    data_cnt = 0
    all_data = []
    all_targets = []
    data_indices = {}
    clients_4_train, clients_4_test = [], []

    train_df = pd.read_csv(train_dir)
    test_df = pd.read_csv(test_dir)

    drop_keys = ['subject_id', 'activity', 'activity_name']

    for client_id in train_df['subject_id'].unique():
        stats[client_id] = {}
        clients_4_train.append(client_id)
        clients_4_test.append(client_id)
        train_data, train_targets = [], []
        test_data, test_targets = [], []
        for row in train_df.to_dict('records'):
            if row['subject_id'] == client_id:
                data = {key: row[key] for key in row.keys() if key not in drop_keys}
                x_train = np.array(list(data.values()))
                y_train = row['activity']
        ...
        client_data = np.append(train_data, test_data, axis=0)
        client_targets = np.append(train_targets, test_targets, axis=0)

        client_data = min_max_scaling(np.array(client_data))

        stats[client_id]["x"] = len(client_data)
        stats[client_id]["y"] = Counter(client_targets)

        all_data.append(client_data)
        all_targets.append(np.array(client_targets))

        data_indices[client_id] = {
            "train": list(range(data_cnt, data_cnt + len(train_data))),
            "val": [],
            "test": list(range(data_cnt + len(train_data), data_cnt + len(client_data))),
        }
        client_cnt += 1
        data_cnt += len(client_data)
        print(f"Processed client {client_id}.")
    ...
    np.save(DATA_ROOT / "har" / "data", np.concatenate(all_data))
    np.save(DATA_ROOT / "har" / "targets", np.concatenate(all_targets))
...
def process_capture24(args, partition: dict, stats: dict):
    train_dir = DATA_ROOT / "capture24" / "capture24" / "train"
    test_dir = DATA_ROOT / "capture24" / "capture24" / "test"
    client_cnt = 0
    data_cnt = 0
    all_data = []
    all_targets = []
    ...
    filename_list = sorted(os.listdir(train_dir))

    for f in filename_list:
        stats[client_cnt] = {}
        clients_4_train.append(client_cnt)
        clients_4_test.append(client_cnt)

        train_data = np.load(train_dir / f)['X']
        train_targets = np.load(train_dir / f)['Y']

        test_data = np.load(test_dir / f)['X']
        test_targets = np.load(test_dir / f)['Y']
    ...
    np.save(DATA_ROOT / "capture24" / "data", np.concatenate(all_data))
    np.save(DATA_ROOT / "capture24" / "targets", np.concatenate(all_targets))
    ...
    args.client_num = client_cnt
    return HAR(
        root=DATA_ROOT / "capture24",
        args=None,
        test_data_transform=None,
        test_target_transform=None,
        train_data_transform=Augment(),
        train_target_transform=None,
    )
