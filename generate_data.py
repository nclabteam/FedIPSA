...
def main(args):
    ...
    if args.dataset == "ucihar":
        dataset = process_ucihar(args, partition, stats)
        partition["val"] = []
    ...
    if args.plot_distribution:
        if args.dataset in ["domain"]:
            ...
            client_ids = range(args.client_num)
            for i, client_id in enumerate(client_ids):
                for j, (_, cnt) in enumerate(stats[client_id]["y"].items()):
                    counts[j][i] = cnt
            plot_distribution(
                client_num=args.client_num,
                label_counts=counts,
            ...
            for client_id in stats.keys():
                if client_id != "samples_per_client":
                    stats[client_id]["y"] = {int(k): v for k, v in stats[client_id]["y"].items()}
            counts = np.zeros((len(dataset.classes), args.client_num), dtype=np.int64)
            client_ids = range(args.client_num)
            for i, client_id in enumerate(client_ids):
