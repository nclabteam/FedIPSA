...
class FedAvgServer:
    ...
    def __init__(self, args: DictConfig, init_trainer=True, init_model=True):
        ...
        if self.args.common.straggler_ratio > 0:
            ...
            elif args.common.straggler_mode == 'drop':
                clients_drop = [False] * normal_num + [True] * straggler_num
                random.shuffle(clients_drop)
                dropped = 0
                for i in range(self.client_num):
                    if clients_drop[i]:
                        self.train_clients = self.train_clients[:i-dropped] + self.train_clients[i-dropped+1:]
                        dropped += 1
        ...
        self.client_sample_stream = [
            random.sample(
                self.train_clients,
                max(1, int(len(self.train_clients) * self.args.common.join_ratio)),
            )
            ...
            for stage in ["before", "after"]:
                if client_side_test_flag:
                    ...
                    if self.train_clients == self.val_clients == self.test_clients:
                        aggregated = self.test_results[self.current_epoch+1]["all_clients"][stage][split]
                    else:
                        aggregated = self.test_results[self.current_epoch+1]["test_clients"][stage][split]
